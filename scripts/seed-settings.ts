...
import { languages, storyRequirementCategories, topicIdeas } from '../src/db/schema';
...
const seedLanguages = [
  { name: 'German', languageCode: 'de-DE', googleCloudTts: true },
  { name: 'Dutch', languageCode: 'nl-NL', googleCloudTts: true },
  { name: 'French', languageCode: 'fr-FR', googleCloudTts: true },
  { name: 'Spanish', languageCode: 'es-ES', googleCloudTts: true },
  { name: 'English', languageCode: 'en-US', googleCloudTts: true },
  { name: 'Italian', languageCode: 'it-IT', googleCloudTts: true },
  { name: 'Portuguese', languageCode: 'pt-BR', googleCloudTts: true },
  { name: 'Chinese', languageCode: 'zh-CN', googleCloudTts: false },
  { name: 'Ukrainian', languageCode: 'uk-UA', googleCloudTts: true },
  { name: 'Polish', languageCode: 'pl-PL', googleCloudTts: true },
];

const seedTopicIdeas = [
...
const seedStoryRequirements = [
  {
    ...
    options: [
      'excitement', 'anger', 'joy', 'pride', 'earnestness', 'dedication',
      'desperation', 'sadness', 'fear', 'hope', 'nostalgia', 'curiosity',
      'melancholy', 'wonder', 'anxiety', 'serenity', 'anticipation', 'tension',
      'relief', 'confusion', 'determination', 'loneliness', 'contentment',
      'regret', 'euphoria', 'suspense', 'gratitude',
    ],
  ...
  },
  {
    ...
    options: [
      'Comedy', 'Drama', 'Thriller', 'Horror', 'Romance', 'Science Fiction',
      'Fantasy', 'Historical', 'Biography', 'Essay', 'News Article',
      'Travel Guide', 'Travel Diary',
    ],
...
async function seed() {
  ...
  console.log('Seeding topic ideas...');
  for (const topic of seedTopicIdeas) {
    await db.insert(topicIdeas).values({ topic }).onConflictDoNothing();
  }

  console.log('Seeding story requirement categories...');
  for (const req of seedStoryRequirements) {
    await db.insert(storyRequirementCategories).values(req).onConflictDoNothing();
  }
